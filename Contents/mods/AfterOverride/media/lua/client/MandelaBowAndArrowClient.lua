...
function Client.OnGameBoot()
	...
	local archer = TraitFactory.addTrait("MBAOarcher", getText("UI_trait_archer"), 6, getText("UI_trait_archerdesc"), false)
...
function Client.onNewGame(player, square)
	if player:HasTrait("MBAOarcher") then
		local inventory = player:getInventory();
		--local modData = Shared.getModData(player);
		--inventory:AddItem("MandelaBowAndArrow.CompoundBowScorpion"); [ADDITEM]
